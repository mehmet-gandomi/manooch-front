...
const AdminCategoryRow = ({ category, isSelected, onToggleSelect, onEdit }) => {
  return (
          ...
          <div className="flex flex-row items-center justify-between gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {category.hasImage ? (
                  <AdminCategoryStatusChip icon={imageIcon} label="تصویر دارد" />
                ) : null}

                {category.hasDescription ? (
                  <AdminCategoryStatusChip icon={textIcon} label="توضیحات دارد" tone="success" />
                ) : null}

                {category.hasIcon ? (
                  <AdminCategoryStatusChip icon={pathIcon} label="ایکن دارد" />
                ) : null}
              </div>
            </div>
          </div>
